/**
 * Import COR Occupations from Word XML file in batches with progress tracking
 * 
 * This script processes the Word XML file and imports occupations in smaller batches,
 * saving progress to a file so the import can be resumed if interrupted.
 */
...
function loadProgress() {
  if (fs.existsSync(PROGRESS_FILE)) {
    try {
      ...
      console.log(`Loaded progress: processed ${progress.processedBatches} batches`);
      return progress;
    } catch (error) {
      console.error('Error loading progress file:', error);
      return { processedBatches: 0, successCount: 0, failureCount: 0 };
    }
  }
  return { processedBatches: 0, successCount: 0, failureCount: 0 };
}
...
function saveProgress(progress) {
  try {
    ...
    console.log(`Saved progress: processed ${progress.processedBatches} batches`);
  } catch (error) {
...
async function importOccupationsInBatches() {
  try {
    ...
    let { processedBatches, successCount, failureCount } = progress;
    
    // Process remaining occupations in batches
    for (let i = processedBatches * BATCH_SIZE; i < allOccupations.length; i += BATCH_SIZE) {
      ...
      saveProgress({ processedBatches, successCount, failureCount });
