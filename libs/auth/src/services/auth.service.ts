...
import { User as SelectUser } from '@geniuserp/shared';
...
console.log('JWT_SECRET exists and is being used', 'auth-service');
console.log(`JWT_SECRET masked: ${JWT_SECRET.substring(0, 3)}...${JWT_SECRET.substring(JWT_SECRET.length - 3)}`, 'auth-service');
console.log(`JWT_EXPIRES_IN: ${JWT_EXPIRES_IN}`, 'auth-service');
...
export class AuthService {
  ...
  generateToken(user: SelectUser): string {
    ...
    console.log(`Generated token payload: ${JSON.stringify(payload)}`, 'auth-service');
  ...
  async getUserByUsername(username: string): Promise<SelectUser | null> {
    try {
      console.log(`Getting user by username: ${username}`, 'auth-service');
      
      // Use the drizzle auth service if available
      if (this.drizzleService.auth) {
        console.log('Using drizzleService.auth', 'auth-service');
        const user = await this.drizzleService.auth.getUserByUsername(username);
        console.log(`DrizzleService.auth result: ${JSON.stringify(user, null, 2)}`, 'auth-service');
        return user as unknown as SelectUser;
      }
      
      // Direct database query as fallback
      console.log('Using direct database query', 'auth-service');
      ...
      console.log(`Direct query result: ${JSON.stringify(user, null, 2)}`, 'auth-service');
      return user;
    } catch (error) {
      console.error('Error in getUserByUsername:', error);
      // Fall back to storage for backward compatibility
      console.log('Falling back to storage', 'auth-service');
      const user = await storage.getUserByUsername(username);
      console.log(`Storage fallback result: ${JSON.stringify(user, null, 2)}`, 'auth-service');
      return user ?? null;
    }
  }
  ...
  async authenticateUser(username: string, password: string) {
    ...
    console.log(`User retrieved for authentication: ${JSON.stringify(user, null, 2)}`, 'auth-service');
