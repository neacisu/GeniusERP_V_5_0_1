...
export class UserService {
  ...
  async createUser(params: CreateUserParams) {
    try {
      ...
      if (params.roleIds && params.roleIds.length > 0) {
        await this.assignRolesToUser(userId, params.roleIds);
      }
  ...
  async getUsers(options: {
    ...
    try {
      ...
      // Apply sorting
      if (options.sortBy) {
        const column = users[options.sortBy as keyof typeof users] || users.created_at;
  ...
  /**
   * Soft delete a user by changing status to INACTIVE
   * @param userId User ID
   * @returns Updated user record
   */
  ...
  async userHasRole(userId: string, roleIdOrCode: string): Promise<boolean> {
    try {
      ...
      // If not found, try to match by role code
      const role = await this.db.query.roles.findFirst({
        where: eq(roles.code, roleIdOrCode)
      });
      ...
      const userRoleByCode = await this.db.select()
        .from(userRoles)
        .where(and(
          eq(userRoles.user_id, userId),
      ...
      return userRoleByCode.length > 0;
    } catch (error) {
