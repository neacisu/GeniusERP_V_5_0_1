...
export class LicenseService {
  ...
  private async loadActiveLicense(): Promise<void> {
    try {
      const [license] = await this.db.select()
        .from(licenses)
  ...
  async registerLicense(licenseKey: string, actorId: string) {
    try {
      ...
      const existingLicense = await this.db.select()
        .from(licenses)
      ...
      const [license] = await this.db.insert(licenses)
        .values({
          id: uuidv4(),
  ...
  async activateLicense(licenseId: string, actorId: string) {
    try {
      ...
      const [activatedLicense] = await this.db.update(licenses)
        .set({
          status: LicenseStatus.ACTIVE,
          activated_at: new Date(),
          activated_by: actorId,
  ...
  async checkExpiredLicenses(): Promise<void> {
    try {
      ...
      const expiredLicenses = await this.db.select()
        .from(licenses)
        .where(
          and(
