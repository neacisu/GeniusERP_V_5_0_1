...
export class ActivityService {
  ...
  async list(
    ...
    try {
      ...
      if (dateFrom) {
        conditions.push(gte(activities.scheduledDate, dateFrom));
      }

      if (dateTo) {
        conditions.push(lte(activities.scheduledDate, dateTo));
      }
    ...
    try {
      ...
      return await this.db.select()
        ...
        .where(and(
          ...
          eq(activities.assignedTo, userId),
          eq(activities.status, 'pending'),
          lte(activities.scheduledDate, today)
        ))
        .orderBy(asc(activities.scheduledDate));
    } catch (error) {
  ...
  async getActivityStats(companyId: string, userId?: string): Promise<any> {
    try {
      ...
      const statsResult = await this.db.select({
        ...
        today: sql`count(*) filter (where ${activities.scheduledDate} >= ${today} and ${activities.scheduledDate} < ${tomorrow})`,
        overdue: sql`count(*) filter (where ${activities.scheduledDate} < ${today} and ${activities.status} = 'pending')`,
        calls: sql`count(*) filter (where ${activities.type} = 'call')`,
        meetings: sql`count(*) filter (where ${activities.type} = 'meeting')`,
        tasks: sql`count(*) filter (where ${activities.type} = 'task')`,
        emails: sql`count(*) filter (where ${activities.type} = 'email')`
      })
        .from(activities)
        .where(and(...conditions));
