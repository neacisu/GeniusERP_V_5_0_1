...
import { AuthGuard } from '../../auth/guards/auth.guard.fixed'; // Import the fixed AuthGuard
 ...
 */
router.get('/', authGuard.requireAuth(), async (req, res) => {
  try {
 ...
 */
router.get('/:id', authGuard.requireAuth(), async (req, res) => {
  try {
    // Extract user details from JWT token
    const userId = req.user?.id;
    const companyId = req.user?.companyId;
 ...
 */
router.post('/activate-placeholder', AuthGuard.protect(), async (req, res) => {
  try {
    // Extract user details from JWT token
    const userId = req.user?.id;
    const companyId = req.user?.companyId;
